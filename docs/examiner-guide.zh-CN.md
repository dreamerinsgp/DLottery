...
- 提交操作后，点击交易哈希旁的 **View on Etherscan（在 Etherscan 查看）**。等待确认期间即可打开链接；若钱包成功加速交易，链接会更新为确认后的交易哈希。
- 在页面下方打开 **Transactions / Check transactions（查看交易）**，粘贴 Sepolia 交易哈希（0x 后跟 64 位十六进制字符），点击 **Open transaction（打开交易）**，无需连接钱包即可在 Etherscan 查看。
- **Lottery contract activity（抽奖合约活动）**查看抽奖合约相关交易；**USD8 token transfers（USD8 代币转账）**查看代币活动。连接钱包后，可以使用 **My wallet activity（我的钱包活动）**，其中也包含该钱包与 DLottery 无关的活动。
- 在 Etherscan 查看交易状态、发送方、gas 费用和代币转账。授权与购票使用不同的交易哈希。应用显示最近一次提交的哈希；较早的交易可通过钱包活动查找。VRF 请求与后续回调也是两笔独立交易。刚提交的交易可能需要稍等才能显示。
- 区块浏览器链接仅在 Sepolia 部署中提供，本地演示没有公共区块浏览器。
...
网站提供 **Help & common questions（帮助与常见问题）**，可通过页面顶部附近的 **Need help?** 链接进入。点击问题可展开钱包、测试资产、演示参与者、开奖时间、领奖／退款及交易的说明。通过 **English / 中文** 切换帮助语言；网站其他界面仍为英文。无需连接钱包，即使 API 连接不可用也可阅读帮助。演示钱包需要另行准备，帮助区域不会创建钱包或为参与者充值。
