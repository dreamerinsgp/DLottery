...
[English](validation.md) | [简体中文](validation.zh-CN.md) | [Documentation](README.md)

## Public deployment update — 2026-09-14

The Railway frontend, Go backend and PostgreSQL services were deployed, and the test token, lottery and VRF adapter were deployed on Ethereum Sepolia. The production build, 17 contract tests and Go/PostgreSQL race-detector tests passed during deployment. The public browser faucet minted exactly 1,000 test USD8; desktop/mobile checks passed. A real VRF request produced lucky number 9, and the 50 USD8 pool rolled over. API data matched a contract read at the API's indexed block. All three contract sources have exact creation/runtime matches on Sourcify.

See the [deployment report](sepolia-assessment.md), [transaction evidence](sepolia-evidence.json), and [dated API snapshot](review-snapshot.json). These are separate from the earlier local validation below. A public-testnet prize claim and cancellation/refund were not part of the recorded deployment walkthrough; those paths were covered locally. Browser automation used a controlled wallet bridge, not a separately installed MetaMask extension. No screen recording is included; a [recording script](demo-recording.md) is provided.

## Earlier local validation environment
...
## Practical limits of the earlier local validation

- Docker's Linux daemon is not available in this WSL session. Compose was parsed/validated, and the backend was built with the container's CGO setting, but Docker images and `docker compose up` were not executed locally. The later Railway container deployment is recorded separately above.
- At the time of the 2026-09-08 local validation, public credentials were not supplied and provider fulfillment had only been exercised with a local coordinator fixture. The 2026-09-14 Sepolia deployment above supersedes that limitation for Sepolia. BSC testnet deployment remains unverified.
